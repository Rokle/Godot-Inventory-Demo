...
func _ready():
	storage_display = get_parent().find_node("StorageDisplay")
	equipment_display = get_parent().find_node("EquipmentSlotsDisplay")
...
func hover_slot(slot_state,slot):
	if slot_state == "enter" and slot != null:
		...
		hovered_slot = null
		InventoryManager.mouse_slot.show_stats(slot_state, 0)
