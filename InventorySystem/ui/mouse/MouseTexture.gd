...
func divide_items(slot):
	if distribute_content[0] == 0:
		return
	if slot.content[0] != 0 and (slot.content[0] != distribute_content[0] or slot.content[2] != distribute_content[2]):
		return
	...
	for i in range(distribute_items_to.size()):
		if distribute_items_to[i][0] == slot:
			return
	var distribute_amount = int(distribute_content[1] / (distribute_items_to.size()+1)) if division_by_one_item == false else min(1,int(distribute_content[1] / (distribute_items_to.size()+1)))
	...
	if distribute_items_to.size() == 1:
		return
	distributing = true
	var max_amount = ItemData.item_data[str(distribute_content[0])]["max_amount"]
...
func show_stats(slot_state, id):
	stats_id = str(id)
	if slot_state == "exit" or stats_id == "0" or content[0] != 0 or distributing == true:
...
func slot_update(con):
	...
	if content[0] == 0 or content[1] == 0:
		...
		if ItemData.item_data[str(content[0])]["max_amount"] != 1 and content[1] != ItemData.item_data[str(content[0])]["max_amount"]:
